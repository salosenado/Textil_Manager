...
export default function ComprasClienteListScreen({ navigation }) {
  ...
  const loadItems = async () => {
    try {
      const data = await api.getOrdenesCompra({ search, periodo, estado });
      setItems(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  React.useLayoutEffect(() => {
    navigation.setOptions({
      ...
      headerRight: () => (
        <TouchableOpacity onPress={() => navigation.navigate('CompraClienteForm')}>
  ...
  const renderItem = ({ item }) => {
    const proveedorName = item.proveedor_nombre || item.proveedor_nombre_rel || 'Sin proveedor';
    ...
    return (
      <TouchableOpacity
        activeOpacity={0.7}
        style={styles.card}
        onPress={() => navigation.navigate('CompraClienteDetalle', { id: item.id })}
      >
        <View style={styles.cardHeader}>
          <Text style={styles.ventaNum}>OC-{item.numero_compra}</Text>
        ...
        <Text style={styles.clienteName} numberOfLines={1}>{proveedorName}</Text>
        ...
        {item.observaciones ? (
          <Text style={styles.agenteText} numberOfLines={1}>{item.observaciones}</Text>
        ) : null}
      </TouchableOpacity>
    );
  };
      ...
      {items.length === 0 ? (
        <View style={styles.centered}>
          ...
          <Text style={styles.emptyText}>
            {search ? 'Sin resultados' : 'No hay órdenes de compra'}
          </Text>
        </View>
      ) : (
        <FlatList
