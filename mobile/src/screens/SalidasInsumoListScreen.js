...
export default function SalidasInsumoListScreen({ navigation }) {
  ...
  const loadItems = async (searchVal) => {
    try {
      const data = await api.getSalidasInsumo({ search: searchVal ?? debouncedSearch, estado });
      setItems(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  React.useLayoutEffect(() => {
    navigation.setOptions({
      title: 'Salidas de Insumo',
      headerRight: () => (
        <TouchableOpacity onPress={() => navigation.navigate('SalidaInsumoForm')}>
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        activeOpacity={0.7}
        style={styles.card}
        onPress={() => navigation.navigate('SalidaInsumoDetalle', { id: item.id })}
      >
        ...
        {item.destino ? (
          <Text style={styles.captionText} numberOfLines={1}>
            <Ionicons name="location-outline" size={12} color={Colors.textSecondary} /> {item.destino}
          </Text>
        ) : null}
        ...
        <View style={styles.cardFooter}>
          <Text style={styles.dateText}>{formatDate(item.fecha_salida)}</Text>
    ...
    <View style={styles.container}>
      <View style={styles.searchContainer}>
        ...
        <TextInput
          ...
          placeholder="Buscar folio, destino..."
      ...
      {items.length === 0 ? (
        <View style={styles.centered}>
          ...
          <Text style={styles.emptyText}>
            {search ? 'Sin resultados' : 'No hay salidas de insumo'}
          </Text>
        </View>
      ) : (
        <FlatList
