...
struct ComprasClientesListView: View {
        ...
        ScrollView {
            LazyVStack(spacing: 14) {
                ForEach(ordenes) { oc in
                    NavigationLink {
                        OrdenCompraDetalleView(orden: oc)
                    } label: {
                        tarjetaOrden(oc)
                    }
                    .buttonStyle(.plain)
        ...
        return VStack(spacing: 14) {
            ...
            HStack(alignment: .top, spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    filaInfo("Cliente", oc.proveedor)
                    filaInfo("Orden", oc.folio)
                    filaInfo("Artículos", "\(oc.detalles.count)")
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text("\(piezasRecibidas) de \(piezasPedidas) PZ entregadas")
                    .font(.caption)
    ...
    func statusInfo(
        ...
        if pedidas > 0 && recibidas >= pedidas {
            return ("ENTREGA COMPLETA", .green)
        }

        if recibidas > 0 {
            return ("ENTREGA PARCIAL", .orange)
        }
