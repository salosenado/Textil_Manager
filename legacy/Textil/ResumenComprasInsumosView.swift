//
//  ResumenComprasInsumosView.swift
//  Textil
//
...
struct ResumenComprasInsumosView: View {
    ...
    var body: some View {
        ...
        .navigationTitle("Resumen Compras Insumos")
    }
    ...
    var listaOrdenesView: some View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            ForEach(ordenesFiltradas, id: \.persistentModelID) { orden in
                ...
                NavigationLink {
                    DetalleOrdenInsumosView(orden: orden)
                } label: {
                    
                    let porcentaje = porcentajeRecibido(orden)
                    let estado = estadoOrden(orden)
                    let totalOrden = calcularTotalOrden(orden)
                    let totalPagado = calcularTotalPagado(orden)
                    let pendiente = totalOrden - totalPagado
    ...
    var ordenesFiltradas: [OrdenCompra] {
        ...
        var lista = ordenes.filter {
            $0.tipoCompra.lowercased() == "insumo" && !$0.cancelada
        }
