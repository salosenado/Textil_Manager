...
struct AltaCompraInsumosView: View {
    ...
    var body: some View {
        Form {
            ...
            // HEADER
            Section {
                fila("Compra de Insumos", "OCI00-\(numeroOC)")
                fila(
                    "Fecha de compra",
            ...
            Button("Guardar compra de insumos") {
                guardar()
            }
            .disabled(detalles.isEmpty || proveedor == nil)
        }
        .navigationTitle("Compra de Insumos")
        .onAppear {
            calcularConsecutivo()
        }
    }
    ...
    func guardar() {
        ...
        let oc = OrdenCompra(
            ...
            aplicaIVA: aplicaIVA,
            tipoCompra: "insumo",   // 🔴 ESTA ES LA CLAVE
    ...
    func fila(_ t: String, _ v: String) -> some View {
        HStack { Text(t); Spacer(); Text(v).foregroundStyle(.secondary) }
    }
    ...
    func campoNumero(_ t: String, _ b: Binding<String>) -> some View {
        HStack {
            Text(t)
            Spacer()
            TextField("0", text: b)
