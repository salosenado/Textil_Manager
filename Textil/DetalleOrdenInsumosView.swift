...
struct DetalleOrdenInsumosView: View {
    ...
    var body: some View {
        ...
        let recibidosOrden = recibos.filter {
            $0.ordenCompra?.id == orden.id && $0.fechaEliminacion == nil
        }

        let pagosOrden = pagos.filter {
            $0.recibo?.ordenCompra?.id == orden.id && $0.fechaEliminacion == nil
        }
        ...
        .navigationTitle("Detalle orden insumos")
    ...
    func imprimir(_ tipo: Tipo) {
        ...
        let tempURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("OCI_\(orden.numeroOC).pdf")
    ...
    func dibujarOrden(_ y: CGFloat) -> CGFloat {
        ...
        for insumo in orden.detalles {

            // Buscar el modelo real en el catálogo
            let modeloCatalogo = modelos.first {
                $0.nombre == insumo.modelo  // ⚠️ aquí puede ser nombre o codigo según cómo lo guardes
            }

            let descripcion = modeloCatalogo?.descripcion ?? ""

            let linea = """
        Modelo: \(insumo.modelo)\(descripcion.isEmpty ? "" : " – \(descripcion)")
        Cantidad: \(insumo.cantidad) pz
        Costo unitario: \(dinero(insumo.costoUnitario))
        Subtotal: \(dinero(insumo.subtotal))
        """
    ...
    func dibujarRecibido(_ y: CGFloat) -> CGFloat {
        ...
        for r in recibos.filter({ $0.ordenCompra?.id == orden.id && $0.fechaEliminacion == nil }) {
    ...
    func dibujarPagos(_ y: CGFloat) -> CGFloat {
        ...
        for (index, p) in pagos
            .filter({ $0.recibo?.ordenCompra?.id == orden.id && $0.fechaEliminacion == nil })
            .enumerated() {
