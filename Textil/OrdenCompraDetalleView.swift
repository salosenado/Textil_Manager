...
struct OrdenCompraDetalleView: View {
    ...
    // 🔑 RECEPCIONES REALES (CLAVE DEL BLOQUEO)
        ...
        .toolbar {
            if !bloqueada {
                Button(modoEdicion ? "Cerrar" : "Editar") {
                    modoEdicion ? (modoEdicion = false) : (mostrarPassword = true)
                }
            }
        }
    ...
    var observacionesView: some View {
        bloqueGris {
    ...
    var etiqueta: String {
        orden.tipoCompra == "servicio"
        ? "SS-\(String(format: "%03d", orden.numeroOC))"
        : "OC-\(String(format: "%03d", orden.numeroOC))"
    }
