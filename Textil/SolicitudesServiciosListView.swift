//
//  SolicitudesServiciosListView.swift
//  Textil
//
...
struct SolicitudesServiciosListView: View {
    ...
    @Query(
        filter: #Predicate<OrdenCompra> { $0.tipoCompra == "servicio" },
        sort: \.fechaOrden,
        order: .reverse
    )
        ...
        ScrollView {
            LazyVStack(spacing: 14) {
                ForEach(ordenes) { oc in
                    NavigationLink {
                        OrdenServicioDetalleView(orden: oc)
                    } label: {
                        tarjetaOrden(oc)
                    }
                    .buttonStyle(.plain)
        ...
        return VStack(spacing: 14) {
            ...
            HStack(alignment: .top, spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    filaInfo("Servicios", "\(oc.detalles.count)")
